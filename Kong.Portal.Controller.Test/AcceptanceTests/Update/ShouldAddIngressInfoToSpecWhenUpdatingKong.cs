...
public class ShouldAddIngressInfoToSpecWhenUpdatingKong : AcceptanceTest
{
    private KongApi _api;
    private string _json;
    private IApiSwaggerRepository _swaggerRepository;

    public ShouldAddIngressInfoToSpecWhenUpdatingKong()
    {
    ...
    {
        _configuration.Add("ingress-suffix", "henrik.dk");

        var kongRepository = Substitute.For<IKongRepository>();
        kongRepository.Update(Arg.Any<string>(), Arg.Any<string>(), Arg.Do<string>(x => _json = x));
        _registry.AddSingleton(kongRepository);
    ...
    public void AndThenTheProvidedSpecContainsIngressInformation()
    {
        _json.Should().Contain("https://test-api.test-ns.henrik.dk");
    }
}